...
struct BPMPlot: View {
    ...
    // TODO: Draw circles for each stop
    // var stopData: [CGPoint] {
    // }
    // TODO: Snippet to use in LinePlot
    //     ZStack {
    //     ForEach(0..<stopData.count) { x in
...
struct LinePlot: View {
    ...
    private var xMin: CGFloat {
        xmin ?? floor(bpmData.map { $0.x }.min()! / xDiv) * xDiv
    }
    private var xMax: CGFloat {
        xmax ?? ceil(bpmData.map { $0.x }.max()! / xDiv) * xDiv
    }
    private var yMin: CGFloat {
        ymin ?? floor(bpmData.map { $0.y }.min()! / yDiv) * yDiv
    }
    private var yMax: CGFloat {
        ymax ?? ceil(bpmData.map { $0.y }.max()! / yDiv) * yDiv
    }

    private var xDiv: CGFloat {
        xdiv
            ?? {
                let x = bpmData.map { $0.x }
    ...
    private func dataPointToCanvasPoint(
        _ point: CGPoint,
        width: CGFloat = 0,
        height: CGFloat = 0
    ) -> CGPoint {
        ...
        GeometryReader { geo in
            ...
            ZStack(alignment: .topLeading) {
                ...
                Group {
                    ...
                    /* Data */
                    Group {
                        let line = Line(points: bpmData.map(convertPoint))
                        line.path
                            .stroke(.blue)
                    }
                }
